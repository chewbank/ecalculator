...
function arithmetic(...expression) {
   ...
   for (let i = 0; i + 2 < expression.length; i += 2) {
      ...
      const initiative = type(expression[i + 2]);

      let resultLength = 0;
      const resultDecimal = String(result).split('.')[1];
      if (resultDecimal) {
         resultLength = resultDecimal.length;
      }

      let initiativeLength = 0;
      const initiativeDecimal = String(initiative).split('.')[1];
      if (initiativeDecimal) {
         initiativeLength = initiativeDecimal.length;
      }

      const enlarge = Math.pow(10, Math.max(resultLength, initiativeLength));
